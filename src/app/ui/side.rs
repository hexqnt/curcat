...
use super::super::{
    AxisUnit, AxisValueField, CurcatApp, ExportKind, PickMode, PointInputMode, safe_usize_to_f32,
};
use super::common::toggle_switch;
use super::icons;
use crate::interp::InterpAlgorithm;
use crate::snap::{SnapFeatureSource, SnapThresholdKind};
use crate::types::{AxisValue, ScaleKind};
use egui::{
    Color32, CornerRadius, Pos2, Response, RichText, StrokeKind, TextBuffer, TextEdit, Vec2,
...
impl CurcatApp {
    ...
    fn apply_calibration_preset(
        ...
        let (width, height) = match self.image.as_ref() {
            Some(image) => (
        ...
        self.cal_x.v2_text = Self::format_preset_value(span_x * f64::from(x_sign));
        ...
        self.cal_y.v2_text = Self::format_preset_value(span_y * f64::from(y_sign));
