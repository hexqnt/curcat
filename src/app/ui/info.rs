...
impl CurcatApp {
    ...
    pub(crate) fn ui_points_info_window(&mut self, ctx: &egui::Context) {
        if !self.points_info_window_open {
            return;
        }
        ...
        let mut open = self.points_info_window_open;
        egui::Window::new("Points info")
            ...
            .show(ctx, |ui| {
                ...
                self.render_calibration_stats(ui);

                ui.add_space(6.0);
                ui.heading("Geometry");
                self.render_geometry_stats(ui);
            });
        self.points_info_window_open = open;
    }
}
...
impl CurcatApp {
    ...
    fn render_calibration_stats(&self, ui: &mut egui::Ui) {
        let x_len = axis_length(&self.cal_x);
        let y_len = axis_length(&self.cal_y);
        ...
        if let Some(ortho) = self.axis_orthogonality() {
            ...
            ui.label(RichText::new("Add both calibration axes to measure orthogonality.").weak());
        }
    }
    ...
    fn pixel_step_stats(&self) -> Option<(f32, f32)> {
        ...
        let avg = total / ((self.points.len() - 1) as f32);
        Some((avg, total))
    }

    fn axis_orthogonality(&self) -> Option<AxisOrthogonality> {
        let (xp1, xp2) = (self.cal_x.p1?, self.cal_x.p2?);
        let (yp1, yp2) = (self.cal_y.p1?, self.cal_y.p2?);
        let vx = xp2 - xp1;
        let vy = yp2 - yp1;
        let lx = vx.length();
        let ly = vy.length();
