...
5) Экспортируйте результат в CSV/JSON/XLSX.
...
В JSON-файле данные лежат в массиве `points` + служебные `x_unit`/`y_unit`; значения `DateTime` записываются строкой, пустые доп. метрики — `null`.
...
- `src/export.rs` — экспорт CSV/JSON/XLSX.
